...
signupForm.addEventListener('submit', async (event) => {
    ...
    try {
        ...
        window.location.href = 'send_email_verification.html'; // Replace 'otp_verification.html' with your desired webpage URL
    } catch (error) {
...
otpForm.addEventListener('submit', async (event) => {
    ...
    try {
        await makeApiRequest('http://18.117.164.164:4001/api/v1/students/verify_otp', 'POST', { otp });
...
loginForm.addEventListener('submit', async (event) => {
    ...
    try {
        const { accessToken, user } = await makeApiRequest('http://18.117.164.164:4001/api/v1/students/login', 'POST', { email, password });
...
passwordResetForm.addEventListener('submit', async (event) => {
    ...
    try {
        await makeApiRequest('http://18.117.164.164:4001/api/v1/students/send_otp', 'POST', { email });
    } catch (error) {
...
profileForm.addEventListener('submit', async (event) => {
    ...
    try {
        await makeApiRequest('http://18.117.164.164:4001/api/v1/students/update', 'PUT', { name, email }, accessToken);
    } catch (error) {
