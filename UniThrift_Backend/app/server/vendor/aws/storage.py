...
async def upload_file(folder: str, file: UploadFile) -> dict[str, Any]:
    ...
    file_bytes = file.file.read()
    mime_type = file.content_type
    data = await upload_file_with_mime_type(folder, file_bytes, file.filename, mime_type)
    return data


async def upload_file_bytes(folder: str, file_bytes: bytes, file_name, mime_type) -> dict[str, Any]:
    """Uploads file to specific folder on AWS S3
    ...
    Returns:
        dict[str, Any]: response
    """
    data = await upload_file_with_mime_type(folder, file_bytes, file_name, mime_type)
    return data
...
async def upload_file_with_mime_type(folder: str, file_bytes: Union[io.BytesIO, bytes], file_name: str, mime_type: str) -> dict[str, Any]:
    ...
    return {
        'file_name': file_name,
        'mime_type': mime_type,
        'file_size': len(file_bytes.getbuffer()) if isinstance(file_bytes, io.BytesIO) else len(file_bytes),
        'key': filepath,
        'file': f'https://{config.AWS_STORAGE_BUCKET}.s3.{config.AWS_REGION}.amazonaws.com/{filepath}',
    }
...
async def get_file_from_presigned_url(presigned_url):
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(presigned_url) as response:
                ...
                return file_bytes, response.content_type

    except aiohttp.ClientError as error:
        raise HTTPException(status_code=404, detail=f'{error}') from error
