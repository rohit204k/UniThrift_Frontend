...
storage_logger = logger.bind(vendor='Azure-Blob-Storage')
...
async def _blob_sas(container_name: str, blob_name: str, expires_delta: timedelta = timedelta(minutes=5), **kwargs) -> str:
    ...
    try:
        ...
        storage_logger.exception(error)
...
async def generate_download_url(file_key: str, is_private: bool = True):
    """
    Generates a URL for a file based on its file key.

    Args:
        file_key (str): The file key.
        is_private (bool, optional): Whether the file is private or public. Defaults to True.

    Returns:
        str: The generated URL for the file.
    """
    container_name = config.AZURE_PRIVATE_BLOB_CONTAINER_NAME if is_private else config.AZURE_PUBLIC_BLOB_CONTAINER_NAME
    try:
        # If the file key is a URL, extract the blob name from it
...
async def verify_blob(file_key: str, allowed_content_types: list[str] = None, is_private: bool = True):
    ...
    for allowed_type in allowed_content_types:
        # Replace wildcard character with regex equivalent
        pattern = '^' + allowed_type.replace('*', '.*') + '$'
